local is_split_sentence
local wordninja_split

local function capture(cmd)
   local f = assert(io.popen(cmd, 'r'))
   local s = assert(f:read('*a'))
   f:close()
   return s
end

local function init(env)
   local function _pass_func(sentence) return sentence end
   is_split_sentence = env.engine.schema.config:get_bool('configuration/split_sentence')
   if not is_split_sentence then
      wordninja_split = _pass_func
      return
   end
-- configuration 对应 flypy 里的 configuration 主键
   local use_wordninja_rs_lua_module = env.engine.schema.config:get_bool('configuration/use_wordninja_rs_lua_module')
   local use_wordninja_rs = env.engine.schema.config:get_bool('configuration/use_wordninja_rs')
   local use_wordninja_py = env.engine.schema.config:get_bool('configuration/use_wordninja_py')
   local use_wordninja_lua = env.engine.schema.config:get_bool('configuration/use_wordninja_lua')
   if (not use_wordninja_rs_lua_module) and (not use_wordninja_rs) and (not use_wordninja_py) and (not use_wordninja_lua) then
      -- default use wordninja_rs_lua_module
      use_wordninja_rs_lua_module = true
   end

   if use_wordninja_rs_lua_module then
      local wordninja_rs_lua_module_path = env.engine.schema.config:get_string('configuration/wordninja_rs_lua_module_path')
      if  wordninja_rs_lua_module_path and not string.find(package.cpath, wordninja_rs_lua_module_path, 1, true) then
         package.cpath = package.cpath .. ";" .. wordninja_rs_lua_module_path
      end
      local ok,res= pcall(require, 'wordninja')
      wordninja_split = ok and res.split or _pass_func
      return
   end

   if use_wordninja_rs then
      local wordninja_rs_path = env.engine.schema.config:get_string('configuration/wordninja_rs_path')
      wordninja_split = function(sentence)
         return capture(wordninja_rs_path .. " -n '" .. sentence .. "'")
      end
      return
   end

   if use_wordninja_py then
      wordninja_split = function(sentence)
         return capture([[python -c "import sys; import wordninja; sys.stdout.write(' '.join(wordninja.split(']] .. sentence .. [[')))"]])
      end
      return
   end
   if use_wordninja_lua then
     local wordninja_lua= require('tools/wordninja')
     --  init( main_dict, ... ) -- main_dict = nil  default 'wordninja_words.txt'
     wordninja_lua.init(nil, 'ext_words.txt')
     wordninja_split= wordninja_lua.split
     return
   end

end

local function enhance_filter(input, env)
   local cands = {}

   for cand in input:iter() do
      if (cand.comment:find("☯")) then
         if (is_split_sentence) then
            sentence = wordninja_split(cand.text)
            lower_sentence = string.lower(sentence)

            if (not (lower_sentence == sentence)) then
               yield(Candidate("sentence", cand.start, cand._end, lower_sentence .. "", "💡"))
            end

            yield(Candidate("sentence", cand.start, cand._end, sentence .. "", "💡"))
         end
      else
         yield(cand)
      end
   end
end

return { enhance_filter = { init = init, func = enhance_filter} }
